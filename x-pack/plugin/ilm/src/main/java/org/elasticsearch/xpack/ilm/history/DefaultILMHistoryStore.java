...
/**
 * The {@link DefaultILMHistoryStore} handles indexing {@link ILMHistoryItem} documents into the
 ...
 */
public class DefaultILMHistoryStore implements ILMHistoryStore {
    private static final Logger logger = LogManager.getLogger(DefaultILMHistoryStore.class);

    private final boolean ilmHistoryEnabled;
    private final BulkProcessor processor;
    private final ThreadPool threadPool;

    public DefaultILMHistoryStore(Settings nodeSettings, Client client, ClusterService clusterService, ThreadPool threadPool) {
    ...
    public void putAsync(ILMHistoryItem item) {
        ...
        logger.trace("queueing ILM history item for indexing [{}]: [{}]", ILMHistoryStore.ILM_HISTORY_ALIAS, item);
        try (XContentBuilder builder = XContentFactory.jsonBuilder()) {
            item.toXContent(builder, ToXContent.EMPTY_PARAMS);
            IndexRequest request = new IndexRequest(ILMHistoryStore.ILM_HISTORY_ALIAS).source(builder);
            ...
            threadPool.executor(ThreadPool.Names.GENERIC).execute(() -> {
                try {
                    ...
                    logger.error(new ParameterizedMessage("failed add ILM history item to queue for index [{}]: [{}]",
                        ILMHistoryStore.ILM_HISTORY_ALIAS, item), e);
                }
            });
        } catch (IOException exception) {
            logger.error(new ParameterizedMessage("failed to queue ILM history item in index [{}]: [{}]",
                ILMHistoryStore.ILM_HISTORY_ALIAS, item), exception);
        }
    }
    ...
    static void ensureHistoryIndex(Client client, ClusterState state, ActionListener<Boolean> listener) {
        final String initialHistoryIndexName = ILMHistoryStore.ILM_HISTORY_INDEX_PREFIX + "000001";
        final AliasOrIndex ilmHistory = state.metaData().getAliasAndIndexLookup().get(ILMHistoryStore.ILM_HISTORY_ALIAS);
        ...
        if (ilmHistory == null && initialHistoryIndex == null) {
            ...
            client.admin().indices().prepareCreate(initialHistoryIndexName)
                ...
                .addAlias(new Alias(ILMHistoryStore.ILM_HISTORY_ALIAS)
                    .writeIndex(true))
            ...
            listener.onFailure(new IllegalStateException("ILM history index [" + initialHistoryIndexName +
                "] already exists but does not have alias [" + ILMHistoryStore.ILM_HISTORY_ALIAS + "]"));
            ...
            if (((AliasOrIndex.Alias) ilmHistory).getWriteIndex() != null) {
                ...
                listener.onFailure(new IllegalStateException("ILM history alias [" + ILMHistoryStore.ILM_HISTORY_ALIAS + "does not have a write index"));
            }
        } else if (ilmHistory.isAlias() == false) {
            // This is not an alias, error out
            listener.onFailure(new IllegalStateException("ILM history alias [" + ILMHistoryStore.ILM_HISTORY_ALIAS +
                "] already exists as concrete index"));
        } else {
            logger.error("unexpected IndexOrAlias for [{}]: [{}]", ILMHistoryStore.ILM_HISTORY_ALIAS, ilmHistory);
            assert false : ILMHistoryStore.ILM_HISTORY_ALIAS + " cannot be both an alias and not an alias simultaneously";
        }
    }
