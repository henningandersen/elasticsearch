...
public class ShrinkAction implements LifecycleAction {
    ...
    public List<Step> toSteps(IndexLifecycleContext indexLifecycleContext, String phase, StepKey nextStepKey) {
        ...
        WaitForNoFollowersStep waitForNoFollowersStep = new WaitForNoFollowersStep(waitForNoFollowerStepKey, readOnlyKey, indexLifecycleContext);
        UpdateSettingsStep readOnlyStep = new UpdateSettingsStep(readOnlyKey, setSingleNodeKey, indexLifecycleContext, readOnlySettings);
        SetSingleNodeAllocateStep setSingleNodeStep = new SetSingleNodeAllocateStep(setSingleNodeKey, allocationRoutedKey, indexLifecycleContext);
        CheckShrinkReadyStep checkShrinkReadyStep = new CheckShrinkReadyStep(allocationRoutedKey, shrinkKey);
        ShrinkStep shrink = new ShrinkStep(shrinkKey, enoughShardsKey, indexLifecycleContext, numberOfShards, SHRUNKEN_INDEX_PREFIX);
        ...
        ShrinkSetAliasStep aliasSwapAndDelete = new ShrinkSetAliasStep(aliasKey, isShrunkIndexKey, indexLifecycleContext, SHRUNKEN_INDEX_PREFIX);
