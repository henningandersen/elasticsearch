...
public class CorruptedBlobStoreRepositoryIT extends AbstractSnapshotIntegTestCase {
    ...
    public void testConcurrentlyChangeRepositoryContents() throws Exception {
        ...
        final RepositoryData repositoryData = getRepositoryData(
            internalCluster().getMasterNodeInstance(RepositoriesService.class).repository(repoName)
        );
    ...
    public void testFindDanglingLatestGeneration() throws Exception {
        ...
        internalCluster().getCurrentMasterNodeInstance(ClusterService.class)
            .submitStateUpdateTask("set pending generation", new ClusterStateUpdateTask() {
                @Override
    ...
    public void testHandlingMissingRootLevelSnapshotMetadata() throws Exception {
        ...
        Files.write(
            ...
            BytesReference.toBytes(BytesReference.bytes(withoutVersions.snapshotsToXContent(XContentFactory.jsonBuilder(), true))),
                ...
                f -> threadPool.generic()
                    .execute(
                        ActionRunnable.supply(
                            f,
                            () -> snapshotsService.hasOldVersionSnapshots(repoName, getRepositoryData(repository), null)
                        )
                    )
            ),
            is(true)
    ...
    public void testMountCorruptedRepositoryData() throws Exception {
        ...
        assertAcked(
            client.admin()
                .cluster()
                .preparePutRepository(otherRepoName)
                .setType("fs")
                .setSettings(Settings.builder().put("location", repo).put("compress", false))
        );
