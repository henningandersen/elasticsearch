...
public class DocumentParserTests extends ESSingleNodeTestCase {
    ...
    public void testFieldDisabled() throws Exception {
        ...
        BytesReference bytes = BytesReference.bytes(
            XContentFactory.jsonBuilder().startObject().field("foo", "1234").field("bar", 10).endObject()
        );
    ...
    public void testDotsWithFieldDisabled() throws IOException {
        ...
        DocumentMapper mapper = mapperParser.parse("type", new CompressedXContent(mapping));
        {
            BytesReference bytes = BytesReference.bytes(jsonBuilder().startObject().field("foo.bar", 111).endObject());
            ParsedDocument doc = mapper.parse(new SourceToParse("test", "1", bytes, XContentType.JSON));
            assertNull(doc.rootDoc().getField("foo"));
        ...
        {
            BytesReference bytes = BytesReference.bytes(jsonBuilder().startObject().field("foo.bar", new int[] { 1, 2, 3 }).endObject());
            ParsedDocument doc = mapper.parse(new SourceToParse("test", "1", bytes, XContentType.JSON));
            assertNull(doc.rootDoc().getField("foo"));
            assertNull(doc.rootDoc().getField("bar"));
            assertNull(doc.rootDoc().getField("foo.bar"));
        ...
        {
            BytesReference bytes = BytesReference.bytes(
                jsonBuilder().startObject().field("foo.bar", Collections.singletonMap("key", "value")).endObject()
            );
            ParsedDocument doc = mapper.parse(new SourceToParse("test", "1", bytes, XContentType.JSON));
            assertNull(doc.rootDoc().getField("foo"));
            assertNull(doc.rootDoc().getField("bar"));
        ...
        {
            BytesReference bytes = BytesReference.bytes(
                jsonBuilder().startObject().field("foo.bar", "string value").field("blub", 222).endObject()
            );
            ParsedDocument doc = mapper.parse(new SourceToParse("test", "1", bytes, XContentType.JSON));
            assertNull(doc.rootDoc().getField("foo"));
            assertNull(doc.rootDoc().getField("bar"));
    ...
    public void testNestedHaveIdAndTypeFields() throws Exception {
        ...
        ParsedDocument result = mapper.parse(new SourceToParse("index2", "1", BytesReference.bytes(doc), XContentType.JSON));
    ...
    public void testPropagateDynamicWithExistingMapper() throws Exception {
        ...
        DocumentMapper mapper = mapperParser.parse("type", new CompressedXContent(mapping));
        BytesReference bytes = BytesReference.bytes(
            XContentFactory.jsonBuilder().startObject().startObject("foo").field("bar", "something").endObject().endObject()
        );
        ParsedDocument doc = mapper.parse(new SourceToParse("test", "1", bytes, XContentType.JSON));
    ...
    public void testDynamicLongArrayWithTemplate() throws Exception {
        ...
        BytesReference bytes = BytesReference.bytes(
            XContentFactory.jsonBuilder().startObject().startArray("foo").value(0).value(1).endArray().endObject()
        );
        ParsedDocument doc = mapper.parse(new SourceToParse("test", "1", bytes, XContentType.JSON));
    ...
    public void testDynamicFalseLongArray() throws Exception {
        DocumentMapperParser mapperParser = createIndex("test").mapperService().documentMapperParser();
        String mapping = Strings.toString(
            XContentFactory.jsonBuilder().startObject().startObject("type").field("dynamic", "false").endObject().endObject()
        );
        DocumentMapper mapper = mapperParser.parse("type", new CompressedXContent(mapping));
    ...
    public void testDynamicObjectWithTemplate() throws Exception {
        ...
        BytesReference bytes = BytesReference.bytes(
            XContentFactory.jsonBuilder().startObject().startObject("foo").field("bar", "baz").endObject().endObject()
        );
        ParsedDocument doc = mapper.parse(new SourceToParse("test", "1", bytes, XContentType.JSON));
    ...
    public void testDynamicBigInteger() throws Exception {
        ...
        BytesReference bytes = BytesReference.bytes(XContentFactory.jsonBuilder().startObject().field("big-integer", value).endObject());
        ParsedDocument doc = mapper.parse(new SourceToParse("test", "1", bytes, XContentType.JSON));
    ...
    public void testDynamicBigDecimal() throws Exception {
        ...
        DocumentMapper mapper = mapperParser.parse("type", new CompressedXContent(mapping));
        BytesReference bytes = BytesReference.bytes(XContentFactory.jsonBuilder().startObject().field("big-decimal", value).endObject());
        ParsedDocument doc = mapper.parse(new SourceToParse("test", "1", bytes, XContentType.JSON));
    ...
    public void testDynamicDottedFieldNameLongArray() throws Exception {
        ...
        BytesReference bytes = BytesReference.bytes(
            XContentFactory.jsonBuilder().startObject().startArray("foo.bar.baz").value(0).value(1).endArray().endObject()
        );
        ParsedDocument doc = mapper.parse(new SourceToParse("test", "1", bytes, XContentType.JSON));
    ...
    public void testDynamicDottedFieldNameLongArrayWithExistingParentWrongType() throws Exception {
        ...
        assertEquals(
            "Could not dynamically add mapping for field [foo.bar.baz]. "
                + "Existing mapping for [foo] must be of type object but found [long].",
    ...
    public void testDynamicDottedFieldNameObject() throws Exception {
        ...
        BytesReference bytes = BytesReference.bytes(
            XContentFactory.jsonBuilder().startObject().startObject("foo.bar.baz").field("a", 0).endObject().endObject()
        );
        ParsedDocument doc = mapper.parse(new SourceToParse("test", "1", bytes, XContentType.JSON));
    ...
    public void testSimpleMapper() throws Exception {
        ...
        DocumentMapper docMapper = new DocumentMapper.Builder(
            new RootObjectMapper.Builder("person").add(
                new ObjectMapper.Builder("name").add(new TextFieldMapper.Builder("first").store(true).index(false))
            ),
    ...
    public void testDynamicFieldsEmptyName() throws Exception {
        ...
        IllegalArgumentException emptyFieldNameException = expectThrows(
    ...
    public void testBlankFieldNames() throws Exception {
        ...
        final BytesReference bytes2 = BytesReference.bytes(
            XContentFactory.jsonBuilder().startObject().startObject("foo").field("", "bar").endObject().endObject()
        );

        err = expectThrows(MapperParsingException.class, () -> client().prepareIndex("idx").setSource(bytes2, XContentType.JSON).get());
        assertThat(err.getCause(), notNullValue());
        assertThat(err.getCause().getMessage(), containsString("field name cannot be an empty string"));
    }
    ...
    public void testTypeless() throws IOException {
        ...
        BytesReference bytes = BytesReference.bytes(XContentFactory.jsonBuilder().startObject().field("foo", "1234").endObject());
