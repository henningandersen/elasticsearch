...
public class DynamicMappingTests extends ESSingleNodeTestCase {
    ...
    public void testDynamicTrue() throws IOException {
        ...
        ParsedDocument doc = defaultMapper.parse(
            new SourceToParse(
                "test",
                "1",
                BytesReference.bytes(jsonBuilder().startObject().field("field1", "value1").field("field2", "value2").endObject()),
                XContentType.JSON
            )
        );
            ...
            () -> defaultMapper.parse(
                new SourceToParse(
                    ...
                    BytesReference.bytes(
                        XContentFactory.jsonBuilder().startObject().field("field1", "value1").field("field2", (String) null).endObject()
                    ),
                    XContentType.JSON
                )
            )
    ...
    private Mapper parse(DocumentMapper mapper, DocumentMapperParser parser, XContentBuilder builder) throws Exception {
        ...
        SourceToParse source = new SourceToParse("test", "some_id", BytesReference.bytes(builder), builder.contentType());
    ...
    public void testIncremental() throws Exception {
        ...
        Mapper update = parse(
            mapper,
            parser,
            XContentFactory.jsonBuilder().startObject().field("foo", "bar").field("bar", "baz").endObject()
        );
        assertNotNull(update);
        // original mapping not modified
        assertEquals(mapping, serialize(mapper));
    ...
    public void testArray() throws Exception {
        ...
        Mapper update = parse(
            mapper,
            parser,
            XContentFactory.jsonBuilder().startObject().startArray("foo").value("bar").value("baz").endArray().endObject()
        );
        assertNotNull(update);
        // original mapping not modified
        assertEquals(mapping, serialize(mapper));
    ...
    public void testMixTemplateMultiFieldAndMappingReuse() throws Exception {
        ...
        indexService.mapperService()
            .merge("_doc", new CompressedXContent(BytesReference.bytes(mappings1)), MapperService.MergeReason.MAPPING_UPDATE);
        ...
        indexService.mapperService()
            .merge("_doc", new CompressedXContent(parsed.dynamicMappingsUpdate().toString()), MapperService.MergeReason.MAPPING_UPDATE);
    ...
    public void testDefaultFloatingPointMappings() throws IOException {
        ...
        String mapping = Strings.toString(
            jsonBuilder().startObject().startObject("type").field("numeric_detection", true).endObject().endObject()
        );
    ...
    public void testNumericDetectionEnabled() throws Exception {
        String mapping = Strings.toString(
            XContentFactory.jsonBuilder().startObject().startObject("_doc").field("numeric_detection", true).endObject().endObject()
        );
        ...
        ParsedDocument doc = defaultMapper.parse(
            new SourceToParse(
                ...
                "1",
                BytesReference.bytes(
                    XContentFactory.jsonBuilder().startObject().field("s_long", "100").field("s_double", "100.0").endObject()
                ),
                XContentType.JSON
            )
        );
