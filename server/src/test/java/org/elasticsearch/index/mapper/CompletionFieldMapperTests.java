...
public class CompletionFieldMapperTests extends ESSingleNodeTestCase {
    ...
    public void testParsingMinimal() throws Exception {
        ...
        ParsedDocument parsedDocument = defaultMapper.parse(
            new SourceToParse(
                "test",
                "1",
                BytesReference.bytes(XContentFactory.jsonBuilder().startObject().field("completion", "suggestion").endObject()),
                XContentType.JSON
            )
        );
        ...
        MapperParsingException e = expectThrows(
            ...
            () -> defaultMapper.parse(
                new SourceToParse(
                    "test",
                    "1",
                    BytesReference.bytes(XContentFactory.jsonBuilder().startObject().field("completion", 1.0).endObject()),
                    XContentType.JSON
                )
            )
        );
    ...
    public void testKeywordWithSubCompletionAndContext() throws Exception {
        ...
        ParsedDocument parsedDocument = defaultMapper.parse(
            new SourceToParse(
                "test",
                "1",
                BytesReference.bytes(XContentFactory.jsonBuilder().startObject().array("keywordfield", "key1", "key2", "key3").endObject()),
                XContentType.JSON
            )
        );
        ...
        assertThat(
            indexableFields.getFields("keywordfield"),
        ...
        assertThat(
            indexableFields.getFields("keywordfield.subsuggest"),
    ...
    public void testKeywordWithSubCompletionAndStringInsert() throws Exception {
        ...
        assertThat(indexableFields.getFields("geofield.analyzed"), arrayContainingInAnyOrder(suggestField("drm3btev3e86")));
    ...
    public void testCompletionTypeWithSubCompletionFieldAndStringInsert() throws Exception {
        ...
        ParsedDocument parsedDocument = defaultMapper.parse(
            new SourceToParse(
                "test",
                "1",
                BytesReference.bytes(XContentFactory.jsonBuilder().startObject().field("suggest", "suggestion").endObject()),
                XContentType.JSON
            )
        );

        ParseContext.Document indexableFields = parsedDocument.rootDoc();
        assertThat(indexableFields.getFields("suggest"), arrayContainingInAnyOrder(suggestField("suggestion")));
        assertThat(indexableFields.getFields("suggest.subsuggest"), arrayContainingInAnyOrder(suggestField("suggestion")));
    }
    ...
    public void testCompletionTypeWithSubCompletionFieldAndObjectInsert() throws Exception {
        ...
        ParseContext.Document indexableFields = parsedDocument.rootDoc();
        assertThat(indexableFields.getFields("completion"), arrayContainingInAnyOrder(suggestField("New York"), suggestField("NY")));
        assertThat(
            indexableFields.getFields("completion.analyzed"),
    ...
    public void testCompletionTypeWithSubKeywordFieldAndStringInsert() throws Exception {
        ...
        ParseContext.Document indexableFields = parsedDocument.rootDoc();
        assertThat(indexableFields.getFields("completion"), arrayContainingInAnyOrder(suggestField("suggestion")));
        assertThat(
            indexableFields.getFields("completion.analyzed"),
    ...
    public void testParsingMultiValued() throws Exception {
        ...
        ParsedDocument parsedDocument = defaultMapper.parse(
            new SourceToParse(
                ...
                "1",
                BytesReference.bytes(
                    XContentFactory.jsonBuilder().startObject().array("completion", "suggestion1", "suggestion2").endObject()
                ),
                XContentType.JSON
            )
        );
    ...
    public void testFieldValueValidation() throws Exception {
        ...
        try {
            defaultMapper.parse(
                new SourceToParse(
                    ...
                    "1",
                    BytesReference.bytes(
                        XContentFactory.jsonBuilder().startObject().field("completion", charsRefBuilder.get().toString()).endObject()
                    ),
                    XContentType.JSON
                )
            );
        ...
        ParsedDocument doc = defaultMapper.parse(
            new SourceToParse(
                "test",
                "1",
                BytesReference.bytes(XContentFactory.jsonBuilder().startObject().array("completion", "   ", "").endObject()),
                XContentType.JSON
            )
        );
        ...
        ParsedDocument nullDoc = defaultMapper.parse(
            new SourceToParse(
                "test",
                "1",
                BytesReference.bytes(XContentFactory.jsonBuilder().startObject().nullField("completion").endObject()),
                XContentType.JSON
            )
        );
