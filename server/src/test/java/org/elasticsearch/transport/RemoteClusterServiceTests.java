    ...
    public void testRemoteClusterSkipIfDisconnectedSetting() {
        {
            Settings settings = Settings.builder()
                ...
                .put("cluster.remote.bar.skip_unavailable", false)
                .build();
        ...
        {
            Settings brokenSettings = Settings.builder().put("cluster.remote.foo.skip_unavailable", "broken").build();
            expectThrows(
                IllegalArgumentException.class,
        ...
        );
        {
            ...
            assertEquals(
                "missing required setting [cluster.remote.foo.seeds] for setting [cluster.remote.foo.skip_unavailable]",
                iae.getMessage()
            );
        }
        {
            try (MockTransportService remoteSeedTransport = startTransport("seed", new CopyOnWriteArrayList<>(), Version.CURRENT)) {
                ...
                service.validate(
                    Settings.builder()
                        .put("cluster.remote.foo.skip_unavailable", randomBoolean())
                        .put("cluster.remote.foo.seeds", seed)
                        .build(),
                    true
                );
                ...
                AbstractScopedSettings service2 = new ClusterSettings(
                    Settings.builder().put("cluster.remote.foo.seeds", seed).build(),
                    new HashSet<>(
                        Arrays.asList(SniffConnectionStrategy.REMOTE_CLUSTER_SEEDS, RemoteClusterService.REMOTE_CLUSTER_SKIP_UNAVAILABLE)
                    )
                );
