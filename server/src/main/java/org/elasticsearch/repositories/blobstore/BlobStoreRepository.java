...
public abstract class BlobStoreRepository extends AbstractLifecycleComponent implements Repository {
    ...
    protected BlobStoreRepository(
        ...
        indexShardSnapshotFormat = new ChecksumBlobStoreFormat<>(
            SNAPSHOT_CODEC,
        ...
        indexShardSnapshotsFormat = new ChecksumBlobStoreFormat<>(
        ...
        globalMetaDataFormat = new ChecksumBlobStoreFormat<>(
            METADATA_CODEC,
        ...
        indexMetaDataFormat = new ChecksumBlobStoreFormat<>(
        ...
        snapshotFormat = new ChecksumBlobStoreFormat<>(
            SNAPSHOT_CODEC,
            SNAPSHOT_NAME_FORMAT,
    ...
    public void updateState(ClusterState state) {
        ...
        if (bestEffortConsistency) {
            ...
            if (previousBest != metadata.generation()) {
                assert metadata.generation() == RepositoryData.CORRUPTED_REPO_GEN
            ...
            if (latestKnownGen > repositoryStateId) {
                listener.onFailure(
                    new ConcurrentSnapshotExecutionException(
                        new Snapshot(metadata.name(), snapshotId),
                        "Another concurrent operation moved repo generation to [ "
                            + latestKnownGen
                            + "] but this delete assumed generation ["
                            + repositoryStateId
                            + "]"
                    )
                );
        ...
        if (writeShardGens) {
            ...
            writeUpdatedRepoDataStep.whenComplete(updatedRepoData -> {
                ...
                final ActionListener<Void> afterCleanupsListener = new GroupedActionListener<>(
                    ActionListener.wrap(() -> listener.onResponse(null)),
                    2
                );
            ...
            writeIndexGen(updatedRepoData, repositoryStateId, false, ActionListener.wrap(v -> {
                ...
                writeMetaAndComputeDeletesStep.whenComplete(
                    deleteResults -> asyncCleanupUnlinkedShardLevelBlobs(snapshotId, deleteResults, afterCleanupsListener),
    ...
    private void asyncCleanupUnlinkedRootAndIndicesBlobs(
        Map<String, BlobContainer> foundIndices,
        Map<String, BlobMetaData> rootBlobs,
        ...
        for (IndexId indexId : indices) {
            ...
            executor.execute(ActionRunnable.wrap(deleteIndexMetaDataListener, deleteIdxMetaListener -> {
                ...
                try {
                    ...
                    logger.warn(
                        () -> new ParameterizedMessage("[{}] [{}] failed to read metadata for index", snapshotId, indexId.getName()),
                        ex
                    );
                        ...
                        protected void doRun() throws Exception {
                            ...
                            if (useUUIDs) {
                                ...
                                blobStoreIndexShardSnapshots = buildBlobStoreIndexShardSnapshots(
                                    blobs,
                                    shardContainer,
                                    oldRepositoryData.shardGenerations().getShardGen(indexId, shard.getId())
                                ).v1();
                            } else {
    ...
    private List<String> resolveFilesToDelete(SnapshotId snapshotId, Collection<ShardSnapshotMetaDeleteResult> deleteResults) {
        ...
        return Stream.concat(deleteResults.stream().flatMap(shardResult -> {
            ...
            deleteResults.stream()
                .map(shardResult -> shardResult.indexId)
                .distinct()
                .map(indexId -> indexContainer(indexId).path().buildAsString() + globalMetaDataFormat.blobName(snapshotId.getUUID()))
        ).map(absolutePath -> {
    ...
    protected void writeIndexGen(RepositoryData repositoryData, long expectedGen, boolean writeShardGens, ActionListener<Void> listener) {
        ...
        filterRepositoryDataStep.whenComplete(filteredRepositoryData -> {
            ...
            writeAtomic(
                indexBlob,
                BytesReference.bytes(filteredRepositoryData.snapshotsToXContent(XContentFactory.jsonBuilder(), writeShardGens)),
                true
            );
    ...
    private RepositoryMetaData getRepoMetaData(ClusterState state) {
        final RepositoryMetaData metaData = state.getMetaData()
            .<RepositoriesMetaData>custom(RepositoriesMetaData.TYPE)
            ...
            for (String fileName : fileNames) {
                ...
                if (existingFileInfo == null) {
                    ...
                    BlobStoreIndexShardSnapshot.FileInfo snapshotFileInfo = new BlobStoreIndexShardSnapshot.FileInfo(
                        DATA_BLOB_PREFIX + UUIDs.randomBase64UUID(),
                        md,
                        chunkSize()
                    );
    ...
    private void writeShardIndexBlob(BlobContainer shardContainer, String indexGeneration, BlobStoreIndexShardSnapshots updatedSnapshots)
        throws IOException {
    ...
    private Tuple<BlobStoreIndexShardSnapshots, Long> buildBlobStoreIndexShardSnapshots(Set<String> blobs, BlobContainer shardContainer)
        ...
        if (latest >= 0) {
            ...
            .anyMatch(b -> b.startsWith(SNAPSHOT_PREFIX) || b.startsWith(INDEX_FILE_PREFIX) || b.startsWith(DATA_BLOB_PREFIX))) {
                throw new IllegalStateException(
        ...
        try (IndexInput indexInput = store.openVerifyingInput(file, IOContext.READONCE, fileInfo.metadata())) {
            for (int i = 0; i < fileInfo.numberOfParts(); i++) {
                ...
                final InputStream inputStream = new FilterInputStream(
                    maybeRateLimit(new InputStreamIndexInput(indexInput, partBytes), snapshotRateLimiter, snapshotRateLimitingTimeInNanos)
                ) {
                    @Override
